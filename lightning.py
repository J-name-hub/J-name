...
YEONGJONG_BOUNDARY = [
    (45.5000, 135.5000), (35, 135.5000), (35, 120.4333), (45.5000, 120.4333), (45.5000, 135.5000)
]
...
if time_selection == 'All':
    ...
    # 낙뢰가 있는 시간만 추출
    lightning_times = sorted(set([datetime.strptime(item.find('dateTime').text, "%Y%m%d%H%M%S").replace(tzinfo=korea_tz) for item in filtered_data]))
    ...
    # 선택된 시간에 따라 데이터 필터링
    time_filtered_data = [item for item in filtered_data if abs((datetime.strptime(item.find('dateTime').text, "%Y%m%d%H%M%S").replace(tzinfo=korea_tz) - selected_time).total_seconds()) < 1800]  # 30분 이내

# 영종도 관련 옵션에 대한 시간별 낙뢰 횟수 계산
if map_range in ['영종도 내', '영종도 테두리에서 반경 2km 이내']:
    hourly_data = {}
    for hour in range(24):
        count = len([item for item in filtered_data if datetime.strptime(item.find('dateTime').text, "%Y%m%d%H%M%S").hour == hour])
...
if time_filtered_data:
    ...
    for item in time_filtered_data:
        lat = float(item.find('wgs84Lat').text)
        lon = float(item.find('wgs84Lon').text)
        location = (lat, lon)
