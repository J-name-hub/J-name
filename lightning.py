...
# 영종도의 경계 좌표 (수정됨: 경도, 위도 순서)
YEONGJONG_BOUNDARY = [
    (126.3800, 37.4500), (126.4500, 37.4300), (126.5250, 37.4700), (126.5050, 37.5100),
    (126.4800, 37.5050), (126.4200, 37.5000), (126.3900, 37.4700)
]
...
# 영종도 내 위치인지 확인하는 함수 (수정됨)
def is_within_yeongjong(lat, lon, boundary):
    point = Point(lon, lat)  # 순서 변경: (lon, lat)
...
if filtered_data:
    ...
    if map_range == '영종도 내':
        folium.Polygon(
            locations=[(lat, lon) for lon, lat in YEONGJONG_BOUNDARY],
            color="red",
            fill=True,
            fillColor="red",
            fillOpacity=0.1
        ...
        folium.Polygon(
            locations=[(lat, lon) for lon, lat in buffer_polygon.exterior.coords],
            color="blue",
            fill=True,
            fillColor="blue",
            fillOpacity=0.1
