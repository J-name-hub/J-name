...
def main():
    ...
    if st.button("낙뢰 정보 조회"):
        if api_key:
            ...
            if 'response' in lightning_data and 'body' in lightning_data['response']:
                items = lightning_data['response']['body']['items']['item']
                ...
                st.error("데이터를 가져오는 데 실패했습니다. API 응답을 확인해주세요.")
        else:
